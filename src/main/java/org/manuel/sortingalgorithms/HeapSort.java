...
//  private static final Color ROOT_COLOR = Color.GOLD;
  ...
  private void heapify(Node[] arr, int i, int n) {
    int left = 2 * i + 1;
    int right = 2 * i + 2;
    int max = i;
  ...
  private void heapSort(Node[] arr) {
    ...
    //swap root node with final element, heapify subarray
    for (int i = arr.length - 1; i > 0; i--) {
      transitions.add(colorCNode(arr, HIGHLIGHT_COLOR, 0));
